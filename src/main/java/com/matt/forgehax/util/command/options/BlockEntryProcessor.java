...
public class BlockEntryProcessor {
    public static void buildCollection(ExecuteData data) {
        ...
        if(id && regex) throw new CommandExecuteException("Cannot contain both id and regex flag");
        ...
        if(data.hasOption("bounds")) {
            ...
            data.getOptions("bounds").forEach(p -> {
                ...
                if(mm.length > 1) {
                    ...
                    entries.forEach(entry -> entry.getWritableProperty(BoundProperty.class).add(min, max));
                } else {
                    throw new IllegalArgumentException(String.format("Invalid argument \"%s\" given for bounds option. Should be formatted like min-max", value));
                }
            });
        }
    }
