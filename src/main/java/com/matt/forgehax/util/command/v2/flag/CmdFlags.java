...
public class CmdFlags {
  ...
  public static <T extends Enum<T>> void register(Class<T> enumClass) {
    if (!ICmdFlag.class.isAssignableFrom(enumClass))
      throw new IllegalArgumentException("Flag Enum must extend " + ICmdFlag.class.getSimpleName());
  ...
  public static EnumSet<? extends ICmdFlag> get(final String enumClassName) {
    return (EnumSet<? extends ICmdFlag>)
        REGISTRY
            .entrySet()
            .stream()
  ...
  @SuppressWarnings("unchecked")
  @Nullable
  public static Enum<? extends ICmdFlag> get(
      final String enumClassName, final String enumValueName) {
    EnumSet<? extends ICmdFlag> es = get(enumClassName);
    return es == null
        ? null
        : es.stream().filter(v -> v.name().equals(enumValueName)).findFirst().orElse(null);
  }
