...
public class ParentCmd extends AbstractCmd implements IParentCmd {
    private static final Collection<IArg<?>> ARGUMENTS = Collections.singleton(
            new ArgBuilder<ICmd>()
                    .description("child command")
                    .shortDescription("cmd")
                    .required()
                    .converter(ConverterFactory.<ICmd>newBuilder()
                            .type(ICmd.class)
                            .label("cmd")
                            .comparator(CmdHelper::compare)
                            .valuesOf(
                                    (input, stream) -> stream
                                            .filter(cmd -> cmd.getName().toLowerCase().startsWith(input.toLowerCase()))
                                            .sorted(Comparator.comparing(cmd -> cmd.getName().toLowerCase(), Comparator.<String>comparingInt(name -> StringUtils.getLevenshteinDistance(name, input)).thenComparing(String::compareToIgnoreCase)))
                            )
    ...
    public ParentCmd(String name,
                     @Nullable Collection<String> aliases,
                     String description,
                     @Nullable IParentCmd parent,
                     @Nullable Collection<IOption<?>> options) throws CmdRuntimeException.CreationFailure {
    ...
    protected void checkConflictingChildren(ICmd command) {
        if(children.stream().anyMatch(command::isConflictingWith)) throw new CmdRuntimeException("tried to add command that conflicts with another child");
    }
    ...
    @Override
    public boolean process(String[] args) throws CmdUnknownException, CmdMissingArgumentException, CmdAmbiguousException {
        CmdHelper.requireArgument(args, this, getChildrenArgument());

        String next = CmdHelper.firstOf(args);
        List<ICmd> matching = getChildrenArgument().valuesOf(next, children.stream()).collect(Collectors.toList());

        if(matching.isEmpty())
            throw new CmdUnknownException(this, next);
        else if(matching.size() > 1)
            throw new CmdAmbiguousException(this, next, matching);

        return matching.stream()
                .reduce((a, b) -> {throw new CmdAmbiguousException(this, next, matching);})
                .orElseThrow(() -> new CmdUnknownException(this, next))
                .process(CmdHelper.nextOf(args));
    }
