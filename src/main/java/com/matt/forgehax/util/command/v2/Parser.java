...
public class Parser<E extends ICmd> {
  ...
  protected Parser(E command, String[] args) throws CmdMissingArgumentException {
    this.arguments =
        command.getArguments().stream().map(ArgBuilder::newArgMap).collect(Collectors.toList());

    this.options =
        command.getOptions().stream().map(OptionBuilder::newOptionMap).collect(Collectors.toList());
          ...
          if (!option.isFlag()) {
            if (parse.value != null) {
              ...
              if (isOption(arg)) {
                ...
                if (!getOption(p.name).isPresent()) {
                  option.withInput(
                      arg); // since there is no matching option, assume this was intended to be an
                  // argument
                } else {
    ...
    for (int i = argumentIndex; i < arguments.size(); ++i) {
      ...
      if (arg.isRequired()) throw new CmdMissingArgumentException(command, arg);
      else arg.withDefaultValue();
    }
  }

  public Optional<ArgMap<?>> getArgument(int index) {
    return index > -1 && index < arguments.size()
  ...
  private static String extractOption(String text) {
    return isLongToken(text)
        ? text.substring(2)
        : text.substring(1); // remove the -- or - from the option
  }
