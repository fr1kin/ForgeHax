...
public class MCheckbox extends MBase implements IGuiCheckbox {
    ...
    public void onUpdateSize() {
        super.onUpdateSize();
        setWidth(SurfaceHelper.getStringWidth(getFontRenderer(), getText()) + BUFFER_SIZE + CHECKBOX_SIZE);
    ...
    public void onRenderPreBackground(GuiRenderEvent event) {
        ...
        event.getSurfaceBuilder()
                ...
                .color(getFontColor())
                .text(getText(), getX() + CHECKBOX_SIZE + BUFFER_SIZE, getY() - (fontHeight / 2) + (CHECKBOX_SIZE / 2))
        ...
        if(isHovered() && getHoveredTime() > 10) {
            ...
            event.getSurfaceBuilder()
                    ...
                    .color(Utils.Colors.BLACK)
                    .beginQuads()
                    .rectangle(-2, -2, SurfaceHelper.getStringWidth(getFontRenderer(), getHoverText()) + 4, fontHeight + 4)
                    .end()
                    .task(SurfaceBuilder::enableTexture2D)
                    .task(SurfaceBuilder::enableFontRendering)
                    .fontRenderer(getFontRenderer())
