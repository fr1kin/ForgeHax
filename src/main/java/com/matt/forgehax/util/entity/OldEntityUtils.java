...
public class OldEntityUtils implements Globals {
  ...
  public static boolean isFriendlyMob(Entity entity) {
    return (entity.isCreatureType(EnumCreatureType.CREATURE, false)
            && !OldEntityUtils.isNeutralMob(entity))
        || (entity.isCreatureType(EnumCreatureType.AMBIENT, false) && !isBatsDisabled)
        || entity instanceof EntityVillager
        || entity instanceof EntityIronGolem
        || (isNeutralMob(entity) && !OldEntityUtils.isMobAggressive(entity));
  }
  ...
  public static boolean isHostileMob(Entity entity) {
    return (entity.isCreatureType(EnumCreatureType.MONSTER, false)
            && !OldEntityUtils.isNeutralMob(entity))
        || OldEntityUtils.isMobAggressive(entity);
  }
  ...
  public static int getDrawColor(EntityLivingBase living) {
    if (isPlayer(living)) {
      if (PlayerUtils.isFriend((EntityPlayer) living)) return OldUtils.Colors.GREEN;
      else return OldUtils.Colors.RED;
    } else if (isHostileMob(living)) {
      return OldUtils.Colors.ORANGE;
    } else if (isFriendlyMob(living)) {
      return OldUtils.Colors.GREEN;
    } else {
      return OldUtils.Colors.WHITE;
    }
  }
  ...
  public static boolean isAboveWater(Entity entity, boolean packet) {
    ...
    double y =
        entity.posY
            - (packet
                ? 0.03
                : (OldEntityUtils.isPlayer(entity)
                    ? 0.2
