...
public enum Projectile implements IProjectile {
    ...
    private static final int MAX_ITERATIONS         = 1000; // fail safe to prevent infinite loops. MUST be greater than 1
    ...
    public boolean canHitEntity(Vec3d shooterPos, Entity targetEntity) {
        ...
        for(double force = max; force >= min; force -= min) {
            ...
            if(result == null) return false; // this shouldn't happen, but I put it here to stop intelliJ from complaining
    ...
    private static Vec3d getShootPosFacing(Entity entity, AngleN angleFacing) {
        return getEntityShootPos(entity).subtract(
    ...
    public static Projectile getProjectileByItem(Item item) {
        if(item != null) for(Projectile p : values()) if(p.getItem() != null && p.getItem().equals(item))
            return p;
        return NULL;
    }
