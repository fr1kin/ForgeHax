...
public class MCVersionChecker {
    ...
    private static boolean checkVersion(MCVersions mcVersions) {
        if(mcVersions != null) {
            for(String version : mcVersions.value()) if(Objects.equals(getMcVersion(), version))
                return true;
            return false;
        } else return true; // if it isn't marked with MCVersions just assume its compatible with the current version
    }
