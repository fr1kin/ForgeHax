...
/**
 * Created by Babbaj on 9/4/2017.
 * TODO: check all 4 collision box corners
 */
@RegisterMod
public class AutoBucketFallMod extends ToggleMod {
    ...
    public final Setting<Double> preHeight = getCommandStub().builders().<Double>newSettingBuilder()
            ...
            .defaultTo(10D)
            .build();
    public final Setting<Double> settingFallHeight = getCommandStub().builders().<Double>newSettingBuilder()
            .name("height")
    ...
    private void swap(final int slot, final int hotbarNum) {
        MC.playerController.windowClick(getLocalPlayer().inventoryContainer.windowId, slot, hotbarNum, ClickType.SWAP, getLocalPlayer());
    }
