...
public class BookBot extends ToggleMod {
    ...
    private final Setting<String> name = getCommandStub().builders().<String>newSettingBuilder()
            .name("name")
            .description("Name of the book, use {NUMBER} for the number")
            .defaultTo("Book #{NUMBER}")
            .build();

    private final Setting<String> file = getCommandStub().builders().<String>newSettingBuilder()
            .name("file")
            .description("Name of the file inside the forgehax directory to use")
            .defaultTo("")
            .build();

    private final Setting<Boolean> prettify = getCommandStub().builders().<Boolean>newSettingBuilder()
            .name("prettify")
            .description("Enables word wrapping. Can cause book size to increase dramatically")
            .defaultTo(true)
            .build();

    private final Setting<Long> sleep = getCommandStub().builders().<Long>newSettingBuilder()
            .name("sleep")
    ...
    private static final Collection<Character> CHARS_NO_REPEATING = Lists.newArrayList(' ', '\n', '\t', '\r');
    ...
    private static String parseText(String text, boolean wrap) {
        ...
        for(int i = 0, p = i; i < text.length(); i++, p++, p %= MAX_CHARACTERS_PER_PAGE) {
            ...
            if(wrap
                ...
                // if the word (distance between two spaces) is less than the max chars allowed (to prevent words greater than it from causing an infinite loop), and
    ...
    private BookWriter loadFile() throws RuntimeException {
        ...
        if (f.getName().endsWith(".txt") || f.getName().endsWith(".book")) {
            return new BookWriter(this, f.getName().endsWith(".txt") ? parseText(text, prettify.get()) : text);
        } else
            throw new RuntimeException("File is not of a .txt or .book type");
    }

    @Override
    public String getDisplayText() {
        return this.writer == null ? super.getDisplayText() : super.getDisplayText() + "[" + this.writer.toString() + "]";
    }
    ...
    protected void onLoad() {
        ...
        getCommandStub().builders().newCommandBuilder()
                ...
                .processor(data -> {
                    if(writer != null) {
                        writer.setFinalListener(o -> ConsoleIO.write("BookBot task stopped at page " + writer.getPage()));
                        writer.stop();
                        writerThread = null;
        ...
        getCommandStub().builders().newCommandBuilder()
                ...
                .processor(data -> {
                    ...
                    // optional argument, if not given use name from file variable and rename the extension to .book
                    ...
                    if(writer != null) {
                        try {
                            Files.write(getFileManager().getFileInBaseDirectory(fname).toPath(), writer.contents.getBytes());
    ...
    private static class BookWriter implements Runnable {
        ...
        public void setPage(int page) {
            if(parser != null) throw new RuntimeException("Cannot set position while task is running or stopped");
            this.page = page;
        }
        ...
        public String toString() {
            return String.format("Status=%s,P/T=%d/%d,B/T=%d/%d",
                    status.name(),
                    page, getTotalPages(),
                    getBook(), getTotalBooks()
            );
        }
