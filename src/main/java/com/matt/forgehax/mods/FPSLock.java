...
public class FPSLock extends ToggleMod {
    private final Setting<Integer> fps = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("fps")
            .description("FPS to use when the world is loaded")
            .min(0)
            .defaultTo(60)
            .build();
    private final Setting<Integer> menu_fps = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("menu-fps")
