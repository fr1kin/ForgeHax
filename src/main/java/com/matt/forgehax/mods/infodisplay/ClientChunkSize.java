...
public class ClientChunkSize extends ToggleMod {
  ...
  public ClientChunkSize() {
    super(Category.GUI, "ClientChunkSize", false, "Shows the client-side chunk size in bytes");
  }
  ...
  public String getInfoDisplayText() {
    return super.getDisplayText() + ChatFormatting.GRAY + " [" + ChatFormatting.WHITE + String.format(
        "%s | %s", size == -1 ? "<error>" : toFormattedBytes(size), difference(size - previousSize)) +
