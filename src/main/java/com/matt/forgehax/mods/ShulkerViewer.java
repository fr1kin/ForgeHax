...
public class ShulkerViewer extends ToggleMod {
    ...
    private final Setting<Boolean> help_text = getCommandStub().builders().<Boolean>newSettingBuilder()
            .name("help-text")
            .description("Text to inform new users that the shulker contents can be viewed by holding a key down")
            .defaultTo(true)
            .build();

    private final Setting<Boolean> toggle_lock = getCommandStub().builders().<Boolean>newSettingBuilder()
            .name("toggle-lock")
            .description("GUI will remain locked in place until key is pressed again")
            .defaultTo(false)
            .build();

    private final Setting<Integer> tooltip_opacity = getCommandStub().builders().<Integer>newSettingBuilder()
    ...
    private final Setting<Integer> locked_opacity = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("locked-opacity")
    ...
    private final Setting<Integer> x_offset = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("x-offset")
            .description("X Offset for the tool tip")
            .defaultTo(8)
            .build();

    private final Setting<Integer> y_offset = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("y-offset")
            .description("Y Offset for the tool tip")
            .defaultTo(0)
            .build();

    private final KeyBinding lockDownKey = new KeyBinding("ShulkerViewer Lock", Keyboard.KEY_LSHIFT, "ForgeHax");

    private final List<GuiShulkerViewer> guiCache = Lists.newArrayListWithExpectedSize(CACHE_RESERVE_SIZE);
    ...
    private boolean setInCache(int index, @Nullable GuiShulkerViewer viewer) {
        if(index < 0)
            ...
            if(previous > (CACHE_RESERVE_SIZE - 1) && !getInCache(previous).isPresent()) // check if previous entry is null and remove it recursively if it is
                return setInCache(previous, null);
            else
                return true;
        } else if(index > guiCache.size() - 1) { // array not big enough
            for(int i = Math.max(guiCache.size(), 1); i < index; ++i) guiCache.add(i, null); // fill with nulls up to the index
    ...
    private Optional<GuiShulkerViewer> getInCache(int index) {
        return Utils.isInRange(guiCache, index) ? Optional.ofNullable(guiCache.get(index)) : Optional.empty();
    }

    private void clearCache() {
        for(int i = 0; i < CACHE_RESERVE_SIZE; ++i) setInCache(i, null); // set all reserve slots to null, and add them if they don't already exist
        while(guiCache.size() > CACHE_RESERVE_SIZE) setInCache(guiCache.size() - 1, null); // clear the rest
    }
    ...
    public void onRender(GuiScreenEvent.DrawScreenEvent.Post event) {
        ...
        try {
            ...
            if(!isLocked() || (lastX == -1 && lastY == -1)) {
                ...
                renderY = new AtomicInteger(lastY = event.getMouseY() - (SHULKER_GUI_SIZE * count) / 2 + y_offset.get());
            } else {
        ...
        if(help_text.get()) {
            ...
            SurfaceHelper.drawTextShadow("Hold " + Keyboard.getKeyName(lockDownKey.getKeyCode()) + " to view the tooltips of a Shulker boxes content!",
                    5,
                    resolution.getScaledHeight() - (int)(SurfaceHelper.getStringHeight(null) + 2) * 3 - 2,
                    Colors.RED.toBuffer(),
                    1);
            SurfaceHelper.drawTextShadow("The activation key can be configured under Minecraft's Options -> Controls -> ForgeHax -> ShulkerViewer Lock.",
                    5,
                    resolution.getScaledHeight() - (int)(SurfaceHelper.getStringHeight(null) + 2) * 2 - 2,
                    Colors.GREEN.toBuffer(),
                    1);
            SurfaceHelper.drawTextShadow("Type in chat \"" + ChatCommandService.getActivationCharacter() + getModName() + "\" for more options, and \"" + ChatCommandService.getActivationCharacter() + getModName() + " " + help_text.getName() + " false\" to disable this help message.",
                    5,
    ...
    class GuiShulkerViewer extends GuiContainer implements Comparable<GuiShulkerViewer> {
        ...
        public void drawScreen(int mouseX, int mouseY, float partialTicks) {
            ...
            GlStateManager.disableLighting();
            GlStateManager.color(1.f, 1.f, 1.f, !isLocked() ? (tooltip_opacity.getAsFloat() / 255.f) : (locked_opacity.getAsFloat() / 255.f));
