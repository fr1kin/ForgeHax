...
public class ChunkLogger extends ToggleMod {
    ...
    private final Setting<Integer> max_chunks = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("max-chunks")
            .description("Maximum chunks to render (set to 0 for infinite)")
            .defaultTo(5120)
            .build();

    private final Setting<Boolean> clear_on_toggle = getCommandStub().builders().<Boolean>newSettingBuilder()
            .name("clear-on-toggle")
            .description("Clear chunk list on disable")
            .defaultTo(true)
            .build();

    private final Setting<ShowChunkEnum> show_only = getCommandStub().builders().<ShowChunkEnum>newSettingEnumBuilder()
            .name("show-only")
            .description("Specify which chunk to only show")
            .defaultTo(ShowChunkEnum.ALL)
            .build();

    private final Setting<DetectionMethodEnum> detection_method = getCommandStub().builders().<DetectionMethodEnum>newSettingEnumBuilder()
    ...
    private final Setting<Long> flag_timing = getCommandStub().builders().<Long>newSettingBuilder()
            .name("flag-timing")
            .description("Maximum time in MS that another chunk load in succession will trigger it to be marked as a new chunk")
            .defaultTo(1000L)
            .build();

    private final Setting<Integer> block_change_threshold = getCommandStub().builders().<Integer>newSettingBuilder()
            .name("block-change-threshold")
            .description("Maximum number of blocks required to change between chunk loading in order to be marked as a new chunk")
            .defaultTo(100)
            .build();
    ...
    public void onRender(RenderEvent event) {
        ...
        copy.forEach(chunk -> {
            ...
            GeometryTessellator.drawQuads(event.getBuffer(),
                    chunk.bbox.minX,
                    chunk.bbox.minY,
                    chunk.bbox.minZ,
                    chunk.bbox.maxX,
    ...
    private class ChunkData {
        ...
        ChunkData(SPacketChunkData packet) {
            ...
            bbox = new AxisAlignedBB(pos.getXStart(), 0, pos.getZStart(), pos.getXEnd(), 255, pos.getZEnd());
        ...
        public boolean equals(Object obj) {
            return obj == this
                    || (obj instanceof ChunkData && this.pos.x == ((ChunkData) obj).pos.x && this.pos.z == ((ChunkData) obj).pos.z);
        }
