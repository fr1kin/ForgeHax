...
public class CustomPayloadLogger extends ToggleMod {
    private static final File CLIENT_PAYLOAD_LOG = getFileManager().getFileInBaseDirectory("client2server_payload.log");
    ...
    private void log(Packet packet) {
        if(packet instanceof SPacketCustomPayload) {
            ...
            try {
                Files.write(SERVER_PAYLOAD_LOG.toPath(), input.getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
            } catch (Exception e) {}
        } else if(packet instanceof CPacketCustomPayload) {
            CPacketCustomPayload payloadPacket = (CPacketCustomPayload)packet;
            String input = String.format("%s=%s\n", payloadPacket.getChannelName(), new String(payloadPacket.getBufferData().array()));
            try {
                Files.write(CLIENT_PAYLOAD_LOG.toPath(), input.getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
            } catch (Exception e) {}
        }
    }
