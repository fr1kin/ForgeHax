    ...
    if (obj != null) {
      ...
      do {
        try {
          for (Field field : clazz.getDeclaredFields()) {
            try {
              if (!Modifier.isStatic(field.getModifiers())) {
                ...
                if (dejaVu.contains(value)) json.addProperty(name, Objects.toString(value));
                else json.add(name, objectAsJson(value, dejaVu));
              }
            } catch (Throwable t) {
            }
          }
          ...
          if (!deep || (superClassLimit != null && clazz.equals(superClassLimit))) clazz = null;
