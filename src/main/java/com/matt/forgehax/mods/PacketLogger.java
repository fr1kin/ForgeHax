...
public class PacketLogger extends ToggleMod implements GsonConstant {
    ...
    private static final File INBOUND = getFileManager().getFileInBaseDirectory("logs/packets/IN-" + DATE + ".log");
    private static final File OUTBOUND = getFileManager().getFileInBaseDirectory("logs/packets/OUT-" + DATE + ".log");
    ...
    private final Setting<Boolean> blacklist_on = getCommandStub().builders().<Boolean>newSettingBuilder()
            .name("blacklist_on")
            .description("Enables the blacklist")
            .defaultTo(false)
            .build();

    private final Options<ClassEntry> blacklist = getCommandStub().builders().<ClassEntry>newOptionsBuilder()
            .name("blacklist")
    ...
    protected void onLoad() {
        blacklist.builders().newCommandBuilder()
                .name("add")
                ...
                .processor(data -> {
                    ...
                    Optional<Class<?>> match = getLoadedClasses(Launch.classLoader).stream()
                            .filter(Packet.class::isAssignableFrom)
                            .filter(clazz -> clazz.getCanonicalName().toLowerCase().contains(className))
                            .sorted((o1, o2) -> String.CASE_INSENSITIVE_ORDER.compare(o1.getCanonicalName(), o2.getCanonicalName()))
                            .findFirst();
                    ...
                    if(match.isPresent()) {
                        ...
                        data.write(String.format("Could not find any class name matching \"%s\"", className));
                        data.markFailed();
                    }
                })
        ...
        blacklist.builders().newCommandBuilder()
                ...
                .processor(data -> {
                    ...
                    Optional<ClassEntry> match = blacklist.stream()
                            .filter(entry -> className.contains(entry.getClassName().toLowerCase()))
                            .sorted((o1, o2) -> String.CASE_INSENSITIVE_ORDER.compare(o1.getClassName(), o2.getClassName()))
                            .findFirst();
    ...
    protected void onDisabled() {
        if(stream_packet_in != null) try {
            stream_packet_in.close();
            stream_packet_in = null;
        } catch (Throwable t) {}

        if(stream_packet_out != null) try {
    ...
    private static JsonElement objectToElement(Object obj, boolean deep, @Nullable Class<?> superClassLimit, @Nonnull final List<Object> dejaVu) {
    ...
    private static JsonElement objectAsJson(Object obj, @Nonnull final List<Object> dejaVu) {
        if (obj == null) return new JsonPrimitive("null");
        try {
    ...
    private static Collection<Class<?>> getLoadedClasses(ClassLoader loader) {
        try {
            ...
            while(mclass != ClassLoader.class) mclass = mclass.getSuperclass();
