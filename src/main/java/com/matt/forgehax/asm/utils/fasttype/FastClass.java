...
public class FastClass extends FastType<Class<?>> {
    ...
    public Class<?> getClassHandle() {
        try {
            if(attemptLookup()) return type;
        } catch (Throwable t) {
            if(printOnce.compareAndSet(false, true)) ASMStackLogger.printStackTrace(t);
        }
        return null;
    }
    ...
    protected Class<?> lookup() throws Exception {
        return Class.forName(name.getByStateSafe(RuntimeState.getState()), false, getClass().getClassLoader());
    }
}
