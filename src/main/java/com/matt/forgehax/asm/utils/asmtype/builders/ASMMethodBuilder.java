...
public class ASMMethodBuilder implements ASMCommon {
    ...
    public ASMMethodBuilder setReturnType(String internalClassName) {
        return setReturnType(!Strings.isNullOrEmpty(internalClassName) ? Type.getObjectType(internalClassName) : null);
    }
    ...
    public ASMMethod build() {
        ...
        Objects.requireNonNull(parameterTypes, "Missing method parameters (use emptyParameters() if none are present)");
        Objects.requireNonNull(returnType, "Missing method return type");
        if(auto) attemptAutoAssign();
        return new ASMMethod(parentClass, NameBuilder.create(name, srgName, obfuscatedName), parameterTypes, returnType);
    }
}
