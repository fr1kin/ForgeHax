...
public class ASMField extends ASMClassChild {
    ...
    public String toString() {
        ...
        builder.append(String.format("FIELD[states=%d,maxStates=%d]{", fieldName.getStateCount(), Math.max(fieldName.getStateCount(), type.getStateCount())));
        Iterator<State> it = Arrays.asList(State.values()).iterator();
        boolean needsSeparator = false;
        while(it.hasNext()) {
            State next = it.next();
            if(fieldName.getByState(next) != null ||
                    type.getByState(next) != null) {
