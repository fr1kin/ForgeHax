...
public class ASMMethod extends ASMClassChild {
    ...
    public String getDescriptorByState(State state) {
        return Type.getMethodType(getReturnTypeByState(state), getArgumentTypesByState(state)).getDescriptor();
    }
    ...
    /**
     * Check if the state has a unique signature because a type in the arguments
     * has multiple states
     * @param state
     * @return
     */
    ...
    public String toString() {
        ...
        builder.append(String.format("METHOD[states=%d,maxStates=%d]{", methodName.getStateCount(), maxStates));
