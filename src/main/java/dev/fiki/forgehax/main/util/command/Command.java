...
import dev.fiki.forgehax.main.util.command.exception.CommandBuildException;
import dev.fiki.forgehax.main.util.command.exception.CommandExecuteException;
import dev.fiki.forgehax.main.util.command.exception.CommandParentNonNullException;
...
import dev.fiki.forgehax.main.util.serialization.ISerializer;
...
public class Command implements Comparable<Command>, ISerializer, GsonConstant {
  ...
  private static final Path SETTINGS_DIR = getFileManager().getMkConfigDirectory("settings");
  ...
  public void serialize() {
    if (this instanceof ISerializableJson) {
      ...
      try {
        ...
        getLogger().error("Serialization error", t);
      } finally {
        try {
          sw.close();
        } catch (IOException e) {
