...
public class ESP extends ToggleMod implements Fonts {
  ...
  public final EnumSetting<DrawOptions> mobs_hostile = newEnumSetting(DrawOptions.class)
      .name("hostile")
      .description("Enables hostile mobs")
      .defaultTo(DrawOptions.NAME)
      .build();

  public final EnumSetting<DrawOptions> mobs_friendly = newEnumSetting(DrawOptions.class)
      .name("friendly")
...
//  } // TODO: 1.15 disable nametag rendering
  ...
  private interface IComponent {
     ...
     */
    boolean valid(EnumSetting<DrawOptions> setting);
  }
...
//          .fontRenderer(ARIAL)
      ...
      @Override
      public boolean valid(EnumSetting<DrawOptions> setting) {
        return DrawOptions.DISABLED.compareTo(setting.getValue()) < 0; // DISABLED less than SETTING
      }
    },
    HEALTH {
      @Override
      ...
      @Override
      public boolean valid(EnumSetting<DrawOptions> setting) {
        return DrawOptions.SIMPLE.compareTo(setting.getValue())
    ...
    ITEMS {
      ...
      @Override
      public boolean valid(EnumSetting<DrawOptions> setting) {
        return DrawOptions.ADVANCED.compareTo(setting.getValue())
